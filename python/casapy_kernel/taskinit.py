...
print "loading casac"
from casac import *
print "loading viewertool"
import viewertool
print "loading inspect"
import inspect
...
def __taskinit_casa( ) :
	...
	stacklevel=0
	for k in range(len(a)):
#                print "stack level: " + str(k) + "\t" + a[k][1] + "\t" + str(string.find(a[k][1],"casa_tasks.py"))
		if string.find(a[k][1],"casapy.py") > -1 or \
                   string.find(a[k][1],"casa_tasks.py") > -1 or \
                   string.find(a[k][1],"casa.py") > -1:
			stacklevel=k
			# jagonzal: Take the first level that matches the requirement
			break
#        print "final stack level: ", stacklevel
#	print "contents: ", a[stacklevel][1]
	myf=sys._getframe(stacklevel).f_globals

	if myf.has_key('casa') :
#                print "found casa in taskinit"
		return myf['casa']
	else:
		return { }
